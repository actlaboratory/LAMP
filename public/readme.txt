...
　LAMP Controllerは、ウェブブラウザ上からLAMPを遠隔操作するためのサイトです。基本的な再生制御はもちろん、音量やリピート再生などの設定変更、再生中ファイルの情報閲覧なども行えます。あらかじめフォルダやファイル情報を登録しておくことで、ファイルを指定した再生や、フォルダのプレイリストへの追加もできます。
...
１，メニューバーの「機能」→「LAMP Controller設定」を開きます。LAMPの登録が成立していない場合は、登録画面が表示されます。
２，ユーザー名、パスワードと、識別用にLAMPの名前を入力し、「登録」を選択します。これにより、新たなパソコンで行った登録の場合は、LAMPが新しく登録され、以前利用していたパソコンからの上書き登録の場合は、登録内容が更新されます。登録の結果は、ダイアログで通知されます。
		...
		2-3-3:フォルダ、ファイルの情報管理
　LAMP Controllerから再生したいファイルのあるフォルダを登録しておくことで、ファイルを指定した再生と、フォルダのプレイリスト追加などが、サイト上から行えるようになります。
メニューバーの「機能」→「LAMP Controllerのファイル」より、登録情報を管理できます。前提として、LAMP Controllerからの要求に基づいてLAMPがファイルを探す際、手元の設定ファイルのファイルパスと、サイト上のファイルパスを連結することによってファイルの保存場所を特定していることを念頭に置いておいてください。
LAMP Controllerのファイルには、以下の機能があります。
　関連付けられたフォルダ：手元のLAMPに登録され、LAMP Controllerからの要求を待機しているフォルダリストです。
